...
public class STVisitor implements assignment_twoVisitor
{
	...
	public Object visit(ASTvarDecl node, Object data)
	  ...
	  public Object visit(ASTconstDecl node, Object data)
	  {
	    ...
	    // shift of three to get to next const 
	...
	public Object visit(ASTAssign node, Object data)
	  {
		...
		for(int i = 1; i < numChildren; i++)
		{
		  ...
		  if(node.jjtGetChild(i).jjtAccept(this,null) instanceof Token )
		  {
		    ...
		    // if token is ID and not in ST for global or current
		    ...
		    else
		    {
		      ...
		      else
		      {
		        ...
		        else if((t.kind == 16 || t.kind == 25) && lhType != "boolean")
		        {
		          System.out.println("Invalid type assign: \n\t"+lhToken.image+":"+lhType+" <- "+t.image+":boolean");
		          System.out.println("Error Line: "+t.beginLine+" Column: "+t.beginColumn+"\n");
		          numErrors++;
		        }  
		      }  
		  ...
		  else
		  {
		    ...
		    for(int j = 0; j < lT.size(); j++)
		    {
		      ...
		      //if token is ID and not in ST for global or current scope
		      if(t.kind == 31 && (ST.get(scope).get(t.image) == null && ST.get("Program").get(t.image) == null) )
		      {
		        System.out.println("Identifier: "+t.image+"\n\tNot declared in scope: "+scope+" Or Program");
		        System.out.println("Error Line: "+t.beginLine+" Column: "+t.beginColumn+"\n");
	...
	// add function decl to current scope 
	...
	{
			Token t = (Token)node.jjtGetChild(i).jjtAccept(this,null);
	// if token is ID and not in ST for the global or current 
			if(t.kind == 31 && (ST.get(scope).get(t.image) == null && ST.get("Program").get(t.image) == null))
	{
			  System.out.println("identifier: "+t.image+"\n\tnot declared in scope: "+scope+" Or Program");
			  System.out.println("Error Line: "+t.beginLine+" Column: "+t.beginColumn+"\n");
	...
	public Object visit(ASTcondition node, Object data)
	{
	// if all Identifers are declared, check if they can reach a boolean condition
	...
	// if function its returned as list of tokens so we need to get the function id
	else
	  	t = ((List<Token>)node.jjtGetChild(0).jjtAccept(this,null)).get(0);
	// if not in current scope its in global 
	...
	public Object visit(ASTargList node, Object data)
	  ...
	  {
	    // get number of params the function has in ST
	    int numParams = 0;
